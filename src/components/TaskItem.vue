<template>
  <li class="task">
    <span v-if="editTaskId !== task.id">{{ task.name }}</span>
    ...
    <div class="btnBox" v-if="editTaskId !== task.id">
      <div class="btn" @click="openTaskEdit(task)">
        <svg width="24px" height="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div class="btn" @click="deleteTask(task.id)">
        <svg width="24px" height="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const openTaskEdit = (task: ITaskProps) => {
  editTaskId.value = task.id;
};
const updateTask = (task: ITaskProps) => {
  if (editTaskId.value === task.id) {
    const editedTask = {
      id: task.id,
      name: formData.name,
    };
